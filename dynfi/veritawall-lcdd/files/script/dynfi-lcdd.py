...
def create_widget(sock, screen_name, widget_name, widget_value, y_position):
    sock.sendall('widget_add {0} {1} string\n'.format(
        screen_name, widget_name).encode())
    sock.sendall('widget_set {0} {1} 1 {2} "{3}"\n'.format(
        screen_name, widget_name, y_position, widget_value).encode())


def show_screen(sock, value1, value2="", sleep=5):
    create_screen(sock, "veritawall")
    for i, output in enumerate([value1, value2], start=1):
        create_widget(sock, "veritawall",
                      "veritawall_widget_{}".format(i), output, i)
    time.sleep(sleep)
    sock.sendall('screen_del veritawall\n'.encode())


def get_cpu_usage():
    process = subprocess.Popen(
        ["top", "-b", "-p", "1"], stdout=subprocess.PIPE, encoding="utf-8")
...
def get_ram_usage():
    ram = int(subprocess.check_output(
        ["sysctl", "-n", "hw.realmem"]).decode().strip())
    ram_mbs = ram / (1024 ** 2)
    output = subprocess.check_output(
        ["sysctl", "-n", "vm.stats.vm.v_free_count"]).decode().strip()
...
def get_ips():
    ...
    interfaces = [interface for interface in output1.split()
...
def get_version():
    return subprocess.check_output(["veritawall-version"]).decode("utf-8").strip()


def cpu_screen(sock):
    for _ in range(5):
        show_screen(sock, " VeritaWall Firewall ",
                    f"CPU usage: {get_cpu_usage()}", sleep=1)


def ram_screen(sock):
    for _ in range(5):
        show_screen(sock, " VeritaWall Firewall ",
                    f"RAM usage: {get_ram_usage()}", sleep=1)
...
def hello_screen(sock):
    show_screen(sock, " VeritaWall Firewall ", ">>> veritawall.com")


def version_screen(sock):
    version = get_version()
    show_screen(sock, " VeritaWall Firewall ", f"Ver: {version}")
...
def main():
    ...
    while True:
        hello_screen(sock)
        version_screen(sock)
        if not os.path.exists('/usr/local/etc/veritawall-lcd-simple'):
            cpu_screen(sock)
            ram_screen(sock)
            ips_screen(sock)
